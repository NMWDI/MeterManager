...
import { PictureAsPdf, SellOutlined } from "@mui/icons-material";
import {
  Box,
  Button,
  Card,
...
import { Route } from "@/routes/reports/soldmeters";
...
export const SoldMetersReportView = () => {
  ...
  const reportQuery = useQuery<SoldMetersReport>({
    queryKey: ["Meters", "report", "soldmeters", search],
    queryFn: async () => {
      ...
      const response = await fetch(
        `${API_URL}/meters/sold-report?${params.toString()}`,
        {
      ...
      if (!response.ok) {
        throw new Error("Failed to fetch sold meters report");
      }
  ...
  const downloadPDFMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await fetch(
        `${API_URL}/meters/sold-report/pdf?${params.toString()}`,
        {
          headers: { Authorization: authHeader() },
        },
      );
      ...
      a.href = url;
      a.download = "sold_meters_report.pdf";
      a.click();
  ...
  return (
    <BackgroundBox>
      <Card sx={{ height: "fit-content" }}>
        <CustomCardHeader
          title={<ReportBreadcrumbTitle current="Sold Meters" />}
          icon={SellOutlined}
        />
        <CardContent>
